...
#   investment_decision: {...}
# - 출력:
#   {"report_path": "agents/outputs/reports/<startup>_investment_report_YYYYMMDD_HHMMSS.pdf"}
...
def _register_korean_font_family() -> str:
    candidates = [
        ("KFONT", FONT_DIR / "NotoSansKR-Regular.ttf", FONT_DIR / "NotoSansKR-Bold.ttf"),
        ("KFONT", FONT_DIR / "NanumGothic.ttf", FONT_DIR / "NanumGothicBold.ttf"),
        ("KFONT", FONT_DIR / "malgun.ttf", FONT_DIR / "malgunbd.ttf"),
        ("KFONT", Path("C:/Windows/Fonts/malgun.ttf"), Path("C:/Windows/Fonts/malgunbd.ttf")),
    ]
    for family, reg, bold in candidates:
        try:
            ...
            if not (reg_path.exists() and bold_path.exists()):
                continue
            registerFont(TTFont(f"{family}-Regular", str(reg_path)))
            registerFont(TTFont(f"{family}-Bold", str(bold_path)))
            try:
                registerFontFamily(
                    family,
...
if not kfamily:
    raise RuntimeError(
        "한글 폰트 등록 실패: agents/resources/fonts/에 NotoSansKR-Regular/Bold 또는 "
...
def render_pdf(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    doc = SimpleDocTemplate(str(fpath), pagesize=A4, leftMargin=36, rightMargin=36, topMargin=40, bottomMargin=36)
    story: List[Any] = []
