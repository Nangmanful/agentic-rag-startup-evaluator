...
# 1) 파일 경로 설정
# -----------------------------
RESOURCE_DOCS_DIR = Path(__file__).resolve().parents[1] / "resources" / "docs"
RESOURCE_DOCS_DIR.mkdir(parents=True, exist_ok=True)
DEFAULT_PDFS = sorted(RESOURCE_DOCS_DIR.glob("*.pdf"))

if DEFAULT_PDFS:
    file_path = [str(path) for path in DEFAULT_PDFS]
else:
    file_path = [str(RESOURCE_DOCS_DIR / "health.pdf")]
